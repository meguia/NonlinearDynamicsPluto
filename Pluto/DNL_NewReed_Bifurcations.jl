...
using DifferentialEquations, Plots, Parameters, Setfield, BifurcationKit, BSplineKit, JLD2, DSP
...
function nreed(u,p)
	...
	[
		u[2]
		u[2]*(μ-u[2]^2)-k*u[1]*(1.0+σ*sqrt(k)*u[1]^2)
	]
end    

# ╔═╡ 991398f1-c686-4850-8580-e65a56408653
...
opts_br = ContinuationPar(p_max = 1.5, p_min = -0.1,tol_stability=1e-8,dsmax=0.01,dsmin=1e-4,ds=1e-4,max_steps = 300,n_inversion = 8);

# ╔═╡ 3fdb9050-22b9-4d42-8785-1fb96d08f117
prob = BifurcationProblem(nreed, [-0.1; 0.0], par, (@lens _.μ))

# ╔═╡ e8c305eb-01c3-4f5f-a227-0c716edba03f
br = continuation(prob, PALC(), opts_br, bothside = true)

# ╔═╡ ed2783c8-20dd-4ef9-9035-8c796b0ba61c
br_po = continuation(br, 2, opts_br, PeriodicOrbitOCollProblem(40, 6); record_from_solution = (x, p) -> begin
		xtt = get_periodic_orbit(p.prob, x, p.p)
		return (max = maximum(xtt[1,:]),
				min = minimum(xtt[1,:]),
				period = getperiod(p.prob, x, p.p))
...
begin
	...
	npar = length(param)
	knotes = 2.0 .^(-3:1/6:1)
	freq = zeros((length(knotes),npar))
	ampl = zeros((length(knotes),npar))
	for m = 1:length(knotes)
		freq[m,:] .= sqrt(knotes[m])
		prob_temp = BifurcationProblem(nreed,[1.0; 0.0],setproperties(par,(k=knotes[m])),(@lens _.μ))
		br_temp = continuation(prob_temp, PALC(), opts_br, bothside = true)
		br_po_temp = continuation(br_temp, 2, opts_br, PeriodicOrbitOCollProblem(40, 6) ; 
		...
		ampl_temp = [br_po_temp[n].max-br_po_temp[n].min for n = 1:length(br_po_temp)]
		...
		freq[m,nini:end] = itp_freq.(param[nini:end])
		ampl[m,nini:end] = itp_ampl.(param[nini:end])
	end	
end	

# ╔═╡ 2f669ecf-1496-4027-8c0a-bfcf571972d2
knotes

# ╔═╡ f2b9c73c-5bb2-4f55-aaef-2dbff4a7fe7e
detune = (1 ./sqrt.(knotes) .* freq)';

# ╔═╡ 07e6e3d4-3568-4112-8be3-ccbe53c7beee
begin
	contourf(knotes,param,ampl',lc=:black, c = :heat, title="",xlabel="k",ylabel="μ")
end	

# ╔═╡ a0b3eac9-7fbd-43b2-9c91-d1ed574b7bc8
begin
	contourf(knotes,param,freq',lc=:black, c = :heat, levels=sqrt.(2.0 .^(-3:1/6:1)),title="",xlabel="k",ylabel="μ")
	contour!(knotes,param,freq',lc=:black,lw=2, levels=sqrt.(2.0 .^(-3:2:1)))
	plot!([sqrt(knotes[22]),sqrt(knotes[22])],[0.01,1.49],label="",c=:black)
end	

# ╔═╡ ed440cdf-8051-410a-b3ea-b2dbbaa49d1a
size(ampl)

# ╔═╡ 19e2772c-0c27-4db6-96b1-8f6434f0dc01
begin
	ytiny = 1e-4
	tend = 2500
	trans = zeros(size(freq))
	harm = zeros(size(freq))
	for (ik,k) in enumerate(knotes)
		for (iμ,μ) in enumerate(param)
			solt = solve(ODEProblem(nreed!,[0;ytiny],tend,[μ ,k, 0.15]),Tsit5(),saveat=0.1)
			y = getindex.(solt.u,2)
			if maximum(y) > 10*ytiny
				trans[ik,iμ] = findfirst(y .> 0.95*maximum(y))
				harm[ik,iμ] = meanfreq(y)
			end	
		end
	end
	harmc = 10*(harm./freq)
...
# ╔═╡ ac16564b-45ad-43d6-86fa-406fb8ac5f4a
begin
	f3a = contourf(knotes,param,freq',lc=:black, c = :imola, levels=sqrt.(2.0 .^(-3:1/6:1)),title="Frequency",xlabel="k",ylabel="μ")
	contour!(knotes,param,freq',lc=:black,lw=2, levels=sqrt.(2.0 .^(-3:2:1)))

	f3b = contourf(knotes,param,ampl',lw=0, c = :imola, title="Amplitude",xlabel="k",ylabel="μ")
	
	f3c = contourf(knotes,param,log10.(trans'),c = :imola,lw=0,title="Transient Duration")
	
	f3d = contourf(knotes,param,harmc',c = :imola,lw=0,title="Harmonic Content")
...
# ╔═╡ 9f6df860-1f98-4381-841d-175d99c84cc8
plot(2.31*sqrt.(param),ampl',legend=false)

# ╔═╡ d909a9fc-1b96-4863-b552-a181274b9e07
1/0.433

# ╔═╡ 87f2c041-5aa4-4266-a637-53de7e169b28
begin
	contourf(1:length(knotes),param,12*log2.(detune),lw=0, title="detune en ST σ=$(par.σ)",xlabel="note",ylabel="μ",xticks=1:24,size=(800,400))
	plot!([12,12],[0.01,1.49],label="",c=:black)
end	

# ╔═╡ 5e1e805d-f329-4671-b392-514238f09af1
save("nreed.jld2","mu",param,"knotes",knotes,"ampl",ampl,"freq",freq,"trans",trans,"harmc",harmc)
...
manifest_format = "2.0"
project_hash = "315c51dc729d4f9bfc9b1adfe81de91667ce5358"
...
[[deps.BifurcationKit]]
deps = ["ArnoldiMethod", "Arpack", "BlockArrays", "DataStructures", "Dates", "DocStringExtensions", "FastGaussQuadrature", "ForwardDiff", "IterativeSolvers", "KrylovKit", "LinearAlgebra", "LinearMaps", "Parameters", "PreallocationTools", "Printf", "RecipesBase", "RecursiveArrayTools", "Reexport", "Requires", "SciMLBase", "Setfield", "SparseArrays", "StructArrays"]
git-tree-sha1 = "79dbbd0d208494295efef045a6a3caaa581773ab"
