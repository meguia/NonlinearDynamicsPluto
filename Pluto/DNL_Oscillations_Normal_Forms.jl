...
# ╔═╡ fe027604-4543-45c9-87a9-0b408fc2c436
alg = Tsit5()
...
begin
	k = 2.0
	μ = 0.4
	sol1a = solve(ODEProblem(vreed!,[0.0;1.0],(0.0,5.0),[-0.4,k,0.0]),alg);
	sol1b = solve(ODEProblem(vreed!,[0.0;1.0],(0.0,20.0),[-0.4,k,0.0]),alg);
	sol2a = solve(ODEProblem(vreed!,[0.02;0.0],(0.0,20.0),[μ,k,0.0]),alg);
	sol2b = solve(ODEProblem(vreed!,[0.02;0.0],(0.0,14.0),[μ,k,0.0]),alg);
	sol2c = solve(ODEProblem(vreed!,[0.0;1.0],(0.0,5.8),[μ,k,0.0]),alg);
	sol2d = solve(ODEProblem(vreed!,[0.0;0.727],(0.0,8.5),[μ,k,0.0]),alg);
end;
...
begin
	...
	ζ = 2.0
	ϵ2 = 1.01
	sol3a = solve(ODEProblem(snlc!,[0.02;-π/9.9],(0,40.0),[ζ,ϵ]),alg,saveat=0.05);
	sol3b = solve(ODEProblem(snlc!,[1.5;-π/9.9],(0,0.7),[ζ,ϵ]),alg,saveat=0.05);
	sol3d = solve(ODEProblem(snlc!,[0.02;-π/9.9],(0,2.8),[ζ,ϵ]),alg,saveat=0.05);
	sol3c = solve(ODEProblem(snlc!,[0.02;-π/9.89],(0,40.0),[ζ,ϵ]),alg,saveat=0.05);
	sol3e = solve(ODEProblem(snlc!,[0.02;-π/9.89],(0,26.0),[ζ,ϵ]),alg,saveat=0.05);
	sol4a = solve(ODEProblem(snlc!,[0.02;-π/14],(0,52.0),[ζ,ϵ2]),alg,saveat=0.05);
	sol4b = solve(ODEProblem(snlc!,[1.5;-π/9.9],(0,2.7),[ζ,ϵ2]),alg,saveat=0.05);
	sol4c = solve(ODEProblem(snlc!,[1.0;0.5],(0,50.0),[ζ,ϵ2]),alg,saveat=0.05);
end;
...
begin
	ξ1 = -0.86
	ξ2 = -0.89
	sol5a = solve(ODEProblem(homocl!,[0.02;0.01],(0,15.0),[ξ1]),alg,saveat=0.05);
	sol5b = solve(ODEProblem(homocl!,[1.02;-0.03],(0,58.0),[ξ1]),alg,saveat=0.05);
	sol5c = solve(ODEProblem(homocl!,[0.02;0.01],(0,4.0),[ξ1]),alg,saveat=0.05);
	sol6a = solve(ODEProblem(homocl!,[0.02;0.02],(0,5.0),[ξ2]),alg,saveat=0.05);
	sol6b = solve(ODEProblem(homocl!,[1.02;-0.03],(0,60.0),[ξ2]),alg,saveat=0.05);
	sol_temp = solve(ODEProblem(homocl!,[0.02;0.01],(0,200.0),[ξ2]),alg,saveat=0.05);
	sol6c = solve(ODEProblem(homocl!,sol_temp.u[end],(0,20.0),[ξ2]),alg,saveat=0.05);
end;
