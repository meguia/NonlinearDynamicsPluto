...
function vfolds(u,p)
	...
	[
		u[2]
		-(k+0.1*μ*(k+1))*u[1]+μ*u[2]-u[1]^2*u[2]-f
	]	
end

# ╔═╡ 71656988-ec80-460d-b887-fc2551e34529
par = (μ = 0.1, k= 0.3, f = 0.5)
...
opts_br = ContinuationPar(p_max = 5.0, p_min = par.μ,tol_stability=1e-8,dsmax=0.01,dsmin=1e-4,ds=1e-4,max_steps = 1400,n_inversion = 8);

# ╔═╡ 3fdb9050-22b9-4d42-8785-1fb96d08f117
prob = BifurcationProblem(vfolds, [-0.5; 0.0], par, (@lens _.μ))
...
# ╔═╡ 2be2b4c6-cf17-4dd3-b69b-b41c9bb2fa55
knotes

# ╔═╡ cbf43382-3704-4959-967a-f866d7e6f6ca


# ╔═╡ 1a30d4c2-611a-4d0a-b7ad-422758152468
plot(knotes,(0.5 ./ knotes).^2)

# ╔═╡ d0801afa-74fd-4366-b035-998ca11864a6
begin
	param = 0.0:0.1:4.0 # μ
	kval = 2 .^(-10/6:1/12:2) # k
	fval = 0.5 # f
	npar = length(param)
	freq1 = zeros(length(kval),npar)
	for m = 1:length(kval)
		μini = 0.0
		prob_temp = BifurcationProblem(vfolds,[0.1; 0.0],setproperties(par,(μ=μini,k=kval[m],f=fval)),(@lens _.μ))
		opts_br2 = ContinuationPar(p_max = 4.0, p_min = μini, tol_stability=1e-8, dsmax=0.01, dsmin=1e-4, ds=1e-4, max_steps = 1000, n_inversion = 8);
		br_temp = continuation(prob_temp, PALC(), opts_br2, bothside = true)
		br_po_temp = continuation(br_temp, 2, opts_br2, PeriodicOrbitOCollProblem(40, 6) ; 
		...
		param_temp = [br_po_temp[i].param for i = 1:length(br_po_temp)]
		freq_temp = [2*pi/br_po_temp[i].period for i = 1:length(br_po_temp)]
		nini = findfirst(x -> x>param_temp[1],param)
		itp_freq = interpolate(param_temp, freq_temp, BSplineOrder(4))
		freq1[m,nini:end] = itp_freq.(param[nini:end])
	end	
end	

# ╔═╡ eb6c8374-5c92-4883-aee3-1b665cc6ea13
length(kval)

# ╔═╡ a0b3eac9-7fbd-43b2-9c91-d1ed574b7bc8
begin
	p1 = contourf(param,kval,freq1,lw=0,ylabel="k",xlabel="μ",title="f = 0.5")
	plot!(param,fval ./sqrt.(param),c=:white)
	for k=0.5:0.5:2.5
		plot!(param,0.0*param*(k+1) .+ k)
	end
	p1
end	

# ╔═╡ 3d0cc8e1-9eb5-44ba-939c-6e25864aa090
size(param)
